...
for t = 1:length(condition_names) % 23 total trials to compile
    ...
    if size(subject_info.(condition_names{t}),1) > 1
        ...
        % Cortex 8 adds a "0" ahead of single digit trial numbers, this accomodates it. Cortex 8 was used from P10 onward I believe.
        if exist(fullfile(data_root, "PAINGAIT" + partic, "Trial" + num2str(subject_info.(condition_names{t}).("PAINGAIT" + partic)(1)) + ".mat"),"file") ==0
            path2file = fullfile(data_root, "PAINGAIT" + partic, "Trial0" + num2str(subject_info.(condition_names{t}).("PAINGAIT" + partic)(1)) + ".mat");
        else
